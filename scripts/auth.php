<?php
session_set_cookie_params(0, "/wedmin");
session_start();
...
if ($user = $db->prepare('SELECT user_id, user_pw, user_name FROM users WHERE user_email = ? AND NOT user_type="wedding_guest"')) {
    ...
    if ($user->num_rows > 0) {
        ...
        if (password_verify($_POST['password'], $password)) {
            ...
            //load the cms class and return API status
            $cms = new CMS;
            $cms->api_connect();
            $api_status = $cms->api_status();
            //if api status is not active, return an error and don't login
            if($api_status!="Active"){
                $pw_status="correct";
                $code = 400;
                $msg = "API error: Error code ".$api_status." If this error persists, contact support";
                $response = array("response_code" => $code,"msg"=>$msg, "user_name" => $username, "user_email" => $user_email, "pw_status" => $pw_status, "api_status" => $api_status);
                exit(json_encode($response));
            }
...
$response = array("response_code" => $code, "user_name" => $username, "user_email" => $user_email, "pw_status" => $pw_status, "api_status" => $api_status);
